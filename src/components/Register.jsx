...
function Register() {
    ...
    useEffect(async () => {
        const getFilehash = async () => {
            const file = new Moralis.File("file.json", { base64: btoa(JSON.stringify(mint_params)) });
            await file.saveIPFS();
            setContractCall({ ...initParams, params: { receiver: account, tokenURI: `https://gateway.moralisipfs.com/ipfs/${file.hash()}` } });
